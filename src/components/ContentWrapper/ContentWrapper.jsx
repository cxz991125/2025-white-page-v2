...
const ContentWrapper = (props) => {
  const { headerTitle, headerContent, headerBgColor = '#9f6daf', bannerOptions, remark, subContent, className, bgImg, showHeader=true } = props;
  return (
    <div className={`content-wrapper${className ? ' ' + className : '' }`}>
      ...
      {showHeader ? (<div className="header-content z-10 relative" style={{background: headerBgColor}}>
        {headerContent ?? <span className="header-title relative">{headerTitle}</span>}
      </div>) : null}
